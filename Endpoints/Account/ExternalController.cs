...
namespace Host.Quickstart.Account
{
  ...
  public class ExternalController : Controller
  {
    ...
    public async Task<IActionResult> Callback()
    {
      ...
      await HttpContext.SignInAsync(user.UsersId.ToString(), user.Username, provider, localSignInProps, additionalLocalClaims.ToArray());
    ...
    private (User user, string provider, string providerUserId, IEnumerable<Claim> claims) FindUserFromExternalProvider(AuthenticateResult result)
    {
      ...
      //get users external email
      var externalEmail = externalUser.FindFirst(ClaimTypes.Email)?.Value;

      var provider = result.Properties.Items["scheme"];
      var providerUserId = userIdClaim.Value;

      // find external user
      var user = _userRepository.FindUserByUsernameOrEmail(externalEmail);
